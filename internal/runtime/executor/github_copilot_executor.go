...
func (e *GitHubCopilotExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	if err = acquireGitHubCopilotRateLimit(ctx, e.cfg); err != nil {
		e.recordCopilotUsageRequest(auth, req.Model, "local-rate-limit", "non_streaming", time.Now().UTC(), true)
		return resp, err
	}
	...
	if err != nil {
		...
		e.recordCopilotUsageRequest(auth, req.Model, path, "non_streaming", requestStartedAt, true)
		return resp, err
	}
	defer func() {
	...
	e.recordCopilotUsageRequest(auth, req.Model, path, "non_streaming", requestStartedAt, false)
	e.scheduleCopilotQuotaRefresh(auth)
...
func (e *GitHubCopilotExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	if err = acquireGitHubCopilotRateLimit(ctx, e.cfg); err != nil {
		e.recordCopilotUsageRequest(auth, req.Model, "local-rate-limit", "streaming", time.Now().UTC(), true)
		return nil, err
	}
	...
	if err != nil {
		recordAPIResponseError(ctx, e.cfg, err)
		e.recordCopilotUsageRequest(auth, req.Model, path, "streaming", requestStartedAt, true)
		return nil, err
	}
	...
	e.recordCopilotUsageRequest(auth, req.Model, path, "streaming", requestStartedAt, false)
	e.scheduleCopilotQuotaRefresh(auth)
...
func FetchGitHubCopilotModels(ctx context.Context, auth *cliproxyauth.Auth, cfg *config.Config) []*registry.ModelInfo {
	...
	for _, entry := range entries {
		...
		if static, ok := staticMap[entry.ID]; ok {
			...
			m.SupportedInputModalities = static.SupportedInputModalities
		} else {
		...
		if endpoints := normalizeGitHubCopilotSupportedEndpoints(entry.SupportedEndpoints); len(endpoints) > 0 {
			m.SupportedEndpoints = endpoints
		}

		if supports := entry.Supports(); supports != nil {
			if len(supports.ReasoningEffort) > 0 {
				m.Thinking = &registry.ThinkingSupport{Levels: dedupeGitHubCopilotStrings(supports.ReasoningEffort)}
			} else if supports.MaxThinkingBudget > 0 || supports.MinThinkingBudget > 0 {
				m.Thinking = &registry.ThinkingSupport{
					Min:         supports.MinThinkingBudget,
					Max:         supports.MaxThinkingBudget,
		...
		if limits := entry.Limits(); limits != nil {
			...
			} else if limits.MaxContextWindowTokens > 0 {
				m.ContextLength = limits.MaxContextWindowTokens
			}
...
func (e *GitHubCopilotExecutor) recordCopilotUsageRequest(auth *cliproxyauth.Auth, model, endpoint, responseType string, requestedAt time.Time, failed bool) {
	internalusage.GetCopilotTracker().RecordRequest(internalusage.CopilotRequestRecord{
		Auth:         auth,
		Model:        model,
		Endpoint:     endpoint,
		...
		Failed:       failed,
		Multiplier:   config.GitHubCopilotPremiumModelMultiplier(e.cfg, model),
	})
}

func (e *GitHubCopilotExecutor) scheduleCopilotQuotaRefresh(auth *cliproxyauth.Auth) {
	if auth == nil {
		return
	}
	accessToken := metaStringValue(auth.Metadata, "access_token")
	...
	time.AfterFunc(time.Second, func() {
		...
		copilotClient := copilotauth.NewCopilotAuth(cfg)
		copilotClient.SetHTTPClient(newProxyAwareHTTPClient(ctx, cfg, authCopy, 15*time.Second))
		usageResp, err := copilotClient.GetUsageWithGitHubToken(ctx, accessToken)
		if err != nil {
			log.Debugf("github-copilot: usage refresh failed: %v", err)
			return
		}
		internalusage.GetCopilotTracker().UpdateQuota(authCopy, usageResp)
	})
}

func normalizeGitHubCopilotSupportedEndpoints(endpoints []string) []string {
	if len(endpoints) == 0 {
		return nil
	}
...
func dedupeGitHubCopilotStrings(values []string) []string {
	if len(values) == 0 {
		return nil
	}
